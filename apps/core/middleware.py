...
class DeviceTemplateDetect(object):
    ...
    then from a view if one is trying to load 'accounts/login.html' the
    render_to will turn that into (accounts/login.ios.html, account/login.html)
    ...
    If no routing is specified, the default routing is:
      (?P<key>iPhone|iPad|Android|iPod|IEMobile|BlackBerry|Mobile\sSafari|$)",
       "mobile", "base.mobile.html", "is_mobile")
